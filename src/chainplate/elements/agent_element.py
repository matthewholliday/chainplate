...
class AgentElement(BaseElement):
    def __init__(self,name="Unnamed Agent", goals="default_goal_var", output_var="__payload__", max_iterations=1, execution_id=""):
        super().__init__()
    ...
    def enter(self, message: Message) -> Message:
        ...
        if(goals_value):
            self.agent_data.save_goals(goals_value)
    ...
    def print_agent_output(self, text: str) -> None:
        self.agent_environment.send_to_user(f"[AGENT] {text}")

    def get_user_input(self, prompt: str) -> str:
        return self.agent_environment.get_user_input(f"[AGENT] {prompt}")
